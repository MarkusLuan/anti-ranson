...
def main (pasta_arquivos_infectados: str, dt_infeccao: datetime.date, extensao: str):
    ...
    print(f"{constantes.BLUE}Escaneando arquivos...")
        ...
        for file in files:
            ...
            if os.path.islink(file_path):
               if not os.path.exists(os.readlink(file_path)):
                   print(f"{constantes.RED}O arquivo aponta para um link invalido: {file}")
            ...
            try:
                ...
                if (extensao and file.endswith(extensao)) or dt_modificacao >= dt_infeccao:
                    print(f"{constantes.RED}Arquivo infectado localizado: {file} | Criado em: {dt_alteracao_metadados.isoformat()} | Modificado em: {dt_modificacao.isoformat()}")
                    f_infectados.write(f"{file_path} \n")
                else:
                    if dt_alteracao_metadados >= dt_infeccao:
                        print(f"{constantes.YELLOW}Os metadados do arquivo foram modificados: {file}")
                        f_meta_alterados.write(f"{file_path} \n")
                    
                    print(f"{constantes.GREEN}O arquivo aparentemente está seguro: {file}")
                    f_seguros.write(f"{file_path} \n")
            except Exception as e:
                print(f"{constantes.RED}Ocorreu um erro ao analisar o arquivo: {file}\nConsiderando como corrompido\nErro: {e}\n")
                f_infectados.write(f"{file_path} \n")
                continue
    ...
    print(f"{constantes.BLUE}Escaneamento concluído!")
...
if __name__ == "__main__":
    ...
    try:
        ...
        raise Exception (f"{constantes.RED}Erro: A data de infecção deverá ser passada nesse formato: DIA/MES/ANO.\nExemplo 26/10/2024!")
    
    if not os.path.isdir(args.caminho):
        raise Exception (f"{constantes.RED}Erro: O caminho informado não existe!\nO caminho deve ser a pasta onde os arquivos infectados estão!")
